      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="mb-24">
          {education.map((edu, index) => (
            <div 
              key={index}
              className="group relative bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 md:p-12 hover:bg-white/[0.04] transition-all duration-700"
            >
              <div className="flex flex-col lg:flex-row gap-12">
                <div className="flex-1">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h4 className="text-2xl md:text-3xl font-bold tracking-tight">{edu.degree}</h4>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {edu.courses.map((course) => (
                      <span 
                        key={course}
                        className="px-4 py-1.5 rounded-full border border-white/10 bg-white/5 text-xs font-mono text-slate-300"
                      >
                        {course}
                      </span>
                    ))}
                ...
                <div className="lg:w-1/3 flex flex-col justify-center">
                  <div className="p-8 rounded-[2rem] bg-gradient-to-br from-[#d946ef]/10 to-transparent border border-white/5">
                    <h5 className="text-xs font-black uppercase tracking-[0.2em] text-[#d946ef] mb-6">Academic Merit</h5>
                    {edu.achievements.map((item) => (
                      <div key={item} className="flex items-center gap-4">
                        ...
                        <span className="text-2xl font-bold tracking-tighter">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {certifications.map((cert, index) => (
            <div 
              key={index}
              className="group relative p-8 rounded-[2rem] bg-[#0c0613] border border-white/5 hover:border-[#d946ef]/30 transition-all duration-500 overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-100 transition-opacity">
                <Award size={40} className="text-[#d946ef] -rotate-12" />
              </div>
              
              <h4 className="text-lg font-bold mb-2 pr-8">{cert.title}</h4>
